...
\newcommand{\ProcurantEntity}{ {{- cleaned_data["entite"] -}} }
\newcommand{\ProcurantNumero}{ {{- cleaned_data["numero"] -}} }
...
%-----------------------------------------------%
\begin{letter}{\`{A} l'attention de Pôle Emploi (\ProcurantEntity)}
...
%-% \`{À} remplacer par votre lettre de motivation
    Je suis \if\PersonGender0 mandat\'{e}e \else mandat\'{e} \fi par \ProcurantFullId \if\ChangementPrenom0 qui a r\'{e}cemment obtenu son changement de pr\'{e}nom à l'\'{E}tat-Civil \fi afin d'effectuer les d\'{e}marches de changement \if\ChangementPrenom0 de pr\'{e}nom et \fi de civilit\'{e} dans les registres de Pôle Emploi (\ProcurantEntity).

    \if\ChangementPrenom0
    \medskip

    \ProcurantId, est \if\ProcurantGender0 connue \else connu \fi de vos services sous le nom de \ProcurantOldId {} et a le num\'{e}ro \ProcurantNumero.
